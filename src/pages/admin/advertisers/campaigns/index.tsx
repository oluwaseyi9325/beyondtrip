...
const Security = () => {
  ...
  const pagedData = useMemo(() => {
    ...
    return campaignMockData.slice(start, start + itemsPerPage);
  }, [currentPage, itemsPerPage]);

  const [open, setOpen] = useState(false);
  const [selectedAdvertiser, setSelectedAdvertiser] = useState<CampaignRow | null>(null);

  const handleViewDriver = (driver: CampaignRow) => {
  ...
  return (
    <>
      ...
      />
      <Container title="Advertisers - Campaigns" active="/admin/advertisers/campaigns">
        <div className="py-3 h-full overflow-y-auto scrollbar-none">
         ...
         <div className="flex flex-col md:flex-row items-stretch sm:items-center gap-4 mb-9 rounded-lg bg-[#C5E4FF] p-[30px] sm:p-[42px]">
          ...
          {/* Table */}
          <AdminAdvertisersCampaignsTable data={pagedData} onViewDriver={handleViewDriver} />
          {/* pagination */}
          <div className="mt-4">
            <Pagination
