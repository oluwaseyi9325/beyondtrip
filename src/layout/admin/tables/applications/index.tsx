...
import { useAdmitStudent, useGetApplications } from "@/services/student.service";
...
const ApplicationsTable = () => {
  ...
  const [selectedStudent, setSelectedStudent] = useState<TApplications | null>(null);
  ...
  const handleClickView = (student: TApplications) => {
  ...
  const columns = useApplicationsColumns({
    onAdmit: (id) => {
      admit.mutate(
        { cohortId: latestCohort?.id, studentId: id },
        {
          onSuccess: () => {
