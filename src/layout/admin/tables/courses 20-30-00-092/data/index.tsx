...
export const useCourseColumns = ({ handleClick, total }: Props): TableColumn<TCourse>[] => {
  ...
  return [
    ...
    },
    {
      name: "driver",
      width: "25%",
      selector: (row) => row.drivers?.map((val) => `${val.firstName || ""} ${val.lastName || ""}`).join(", ") ?? "-",
      // selector: (row) => {row.drivers?.map((val,i)=><div>{val.firstName} { val.lastName}</div>)},
      // selector: (row) => row.drivers?.[0]?.firstName ?? "-",
    },
    {
