...
import { useGetStudents, useInviteStudent } from "@/services/student.service";
...
const InviteStudent = ({ open, handleClose, refetch, classId }: TModal) => {
  ...
  const { mutateAsync: inviteStudent, isPending } = useInviteStudent();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await Promise.all(
        selected.map((s: any) => inviteStudent({ classId, studentId: s.id }))
      );
          ...
          <section className="w-full h-[40vh] overflow-y-auto">
            {isLoading ? (
              ...
              <Table<TStudent>
                ...
                data={filteredBySelects as TStudent[]}
                selectableRows
